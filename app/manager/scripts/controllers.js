...
*/
    ...
    .controller('customerListCtrl',['$scope','$rootScope','$resource',function($scope,$rootScope,$resource){
...
//**********************************************************************************************
    ...
    .controller('customCtrl', ['$scope','$rootScope','$resource','$modal','$http','$timeout',function ($scope,$rootScope,$resource,$modal,$http,$timeout) {
...
//*****************************customer*********************************
        ...
        $scope.saveCustomer=function(form){
            if ($scope.customer.vin && $scope.customer.phone && $scope.customer.name){
                Customer.save($scope.customer,function(){afterChangeCustomer()})
            }
        ...
        if ($rootScope.$stateParams.id){
            if ($rootScope.$stateParams.clone){
                JobTicketArch.get({id:$rootScope.$stateParams.id},function(res){
                    ...
                    $scope.jobTicket.resSum='';
                    $scope.jobTicket.resPay='';
        ...
        $scope.saveJobTicket = function(){
            ...
            $scope.jobTicket.resSum= $scope.getTotalJobs()+$scope.getTotalSpark();
            $scope.jobTicket.resPay= $scope.jobTicket.resSum -$scope.getTotalPay();
        ...
        $scope.$on('printTicket',function(){
            ...
            s +='<div class="container" style="font-size: 10px;"><div class="row"><div class="col-lg-8">'+
                '<h3 style="font-size: 16px; width: 50%; float: left;">Предварительная калькудяция для наряд-заказа от  '+moment(order.date).format('lll')+'</h3> '+
            ...
            //console.log(s);
            s +='<table width="100%" cellspacing="0" cellpadding="5">' +
                '<tr><td><b>Всего по наряду '+($scope.getTotalSpark()+$scope.getTotalJobs()).toFixed(2)+' грн</b></td>' +
                '<td><b>Всего оплачено  '+($scope.getTotalPay()).toFixed(2)+' грн</b></td>' +
                '<td><b>Остаток суммы '+($scope.getTotalSpark()+$scope.getTotalJobs()-$scope.getTotalPay()).toFixed(2)+' грн</b></td></tr>'+
                '</table><hr style="margin-top: 0; margin-bottom: 0; border-color: #666">';
            ...
            s += '<p style="text-align: center; margin-top: 10px; margin-bottom: 5px; ">Стоимость работ по ремонту и обслуживанию</p>' +
                ...
                '<thead><tr><th>#</th><th>Наименование</th><th>К-во</th><th>Сумма</th></thead>';
            ...
            s += '<p style="text-align: center; margin-top: 10px; margin-bottom: 5px;">Стоимость запчастей</p>' +
                '<table width="100%" cellspacing="0" cellpadding="5" border="1px">'+
                '<thead><tr><th>#</th><th>Код</th><th>Наименование</th><th>К-во</th><th>Цена</th><th>Сумма</th></thead>';
            for (var i= 0,l=order.sparks.length;i<l;i++){
                var j=order.sparks[i];
                s +='<tr><td> '+i+1+'</td><td>'+j.code+'</td><td>'+j.name+'</td><td>'+j.q+'</td><td>'+j.price.toFixed(2)+'</td><td>'+(j.price*1.0000001).toFixed(2)+' грн.</td></tr>';
            }
            s+='<tr><th></th><th colspan="4">Итого</th><th>'+$scope.getTotalSpark().toFixed(2)+' грн</th></tr></table>';


            s += '<p style="text-align: center; margin-top: 10px; margin-bottom: 5px;">Платежная ведомость</p>' +
                '<table width="100%" cellspacing="0" cellpadding="5" border="1px">'+
                '<thead><tr><th>#</th><th>Дата</th><th>Сумма</th></thead>';
            for (var i= 0,l=order.pay.length;i<l;i++){
                var j=order.pay[i];
                s +='<tr><td> '+i+1+'</td><td>'+moment(j.date).format('lll')+'</td><td>'+(j.val*1.0000001).toFixed(2)+' грн</td></tr>';
            }
            s+='<tr><th></th><th>Итого</th><th>'+$scope.getTotalPay().toFixed(2)+' грн</th></tr></table><br>';
        ...
        $scope.sendEmail = function(){
            function createBody(order){
                ...
                s +='<div class="container"><div class="row"><div class="col-lg-8">'+
                    '<h3>Наряд-заказ от  '+moment(order.date).format('lll')+'</h3> ';
                ...
                s += '<p style="text-align: center; margin-top: 20px;">Стоимость работ по ремонту и обслуживанию</p>' +
                    '<table width="100%" cellspacing="0" cellpadding="5" border="1px">'+
                    '<thead><tr><th>#</th><th>Наименование</th><th>К-во</th><th>Цена</th></thead>';
                for (var i= 0,l=order.jobs.length;i<l;i++){
                    var j=order.jobs[i];
                    console.log(j);
                ...
                s += '<p style="text-align: center; margin-top: 20px;">Стоимость запчастей</p>' +
                    '<table width="100%" cellspacing="0" cellpadding="5" border="1px">'+
                    '<thead><tr><th>#</th><th>Код</th><th>Наименование</th><th>К-во</th><th>Цена</th></thead>';
                for (var i= 0,l=order.sparks.length;i<l;i++){
                    var j=order.sparks[i];
                    s +='<tr><td> '+i+1+'</td><td>'+j.code+'</td><td>'+j.name+'</td><td>'+j.q+'</td><td>'+(j.price*1.0000001).toFixed(2)+' грн.</td></tr>';
                }
                s+='<tr><th></th><th colspan="3">Итого</th><th>'+$scope.getTotalSpark().toFixed(2)+' грн</th></tr></table>';
            ...
            if (confirm("Закрыть наряд-заказ?")){
                ...
                for (var i= 0,l=$scope.jobTicket.jobs.length;i<l;i++){
                    if (!$scope.jobTicket.jobs[i].sum){
                        alert('Не все работы выполнены!!! Введите исполнителей на работы из списка!');
                        return;
                    }
                }
                ...
                $scope.jobTicket.customer=$scope.customer._id;
                $scope.jobTicket.resSum= $scope.getTotalJobs()+$scope.getTotalSpark();
                $scope.jobTicket.resPay=$scope.getTotalPay();
                $scope.buttonDisabled=true;
        ...
        $scope.getTotalJobs = function(){
            var sum = 0;
            if ($scope.jobTicket.jobs && $scope.jobTicket.jobs.length){
                for(var i= 0,l=$scope.jobTicket.jobs.length;i<l;i++){
                    var j = $scope.jobTicket.jobs[i];
                    sum += j.sum || j.norma*j.q*$scope.getTypeRatio(j.jobType)*$scope.normaHour;
                }
            }
        ...
        $scope.addSpark = function (spark){
            ...
            if (spark && spark.code && spark.name && spark.price ){
                $scope.jobTicket.sparks[$scope.jobTicket.sparks.length]={name:spark.name,
                    code:spark.code,price:spark.price,q:1,shipPrice:null,date:null
                }
        ...
        var ModalInstanceCtrl = function ($scope, $http,$modalInstance,searchStr) {
...
*/
            $scope.getNewPriceA_Z = function(n){
                if (n){
                    var a = (parseFloat(n)*$scope.ratioA_Z).toFixed(2);
                    if (a=='NaN'){a='0.00'}
                    return a;
                } else {
                    return '0.00';
            ...
            $scope.getNewPriceS_cars = function(n){
                if (n){
                    var a = (parseFloat(n)*$scope.ratioS_cars).toFixed(2);
                    if (a=='NaN'){a='0.00'}
                    return a;
                } else {
                    return '0.00';
    ...
    .controller('jobTicketArchCtrl',['$scope','$rootScope','$resource',function($scope,$rootScope,$resource){
        var JobTicketArch = $resource('api/jobticketarch/:id',{id:'@_id'});
...
//**********************************************************************************************
    ...
    .controller('jobTicketArchViewCtrl',['$scope','$rootScope','$resource','$timeout',
        function($scope,$rootScope,$resource,$timeout){
        var JobTicketArch = $resource('api/jobticketarch/:id',{id:'@_id'});
        JobTicketArch.get({id:$rootScope.$stateParams.id},function(res){
            //console.log(res);
            $scope.jobTicket = res;
        ...
        $scope.printJobTicket = function(){
            ...
            var s='';
            s +='<div class="container" style="font-size: 10px;"><div class="row"><div class="col-lg-8">'+
                '<h3 style="font-size: 16px; width: 50%; float: left;">Наряд-заказ от  '+moment(order.date).format('lll')+'</h3> '+
                '<h3 style="font-weight: bold; font-size: 26px; margin-top: 10px;">AUTOFAN <span style="font-size: 18px; font-weight: normal">харьков</span></h3>';
                ...
                JobTicketArch.query({dtfrom:start.toString(),dtto:end.toString(),perPage:300,data:'all'},function(res){
                    ...
                    for (var i= 0,l=res.length;i<l;i++){
                        ...
                        for (var j= 0,lj=res[i].sparks.length;j<lj;j++){
                            sum1+=res[i].sparks[j].shipPrice*res[i].sparks[j].q;
                            sum+=res[i].sparks[j].price*res[i].sparks[j].q;
